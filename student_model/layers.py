...
from student_model.aug import sim_global
...
class STEncoder(nn.Module):
    def __init__(self, Kt, Ks, blocks, input_length, num_nodes, droprate=0.1):
        ...
        out_len = input_length - 2 * (Kt - 1) * len(blocks)+4
    ...
    def forward(self, x0, graph):
        ...
        return x,[tc11,tc12,tc13,tc14] # nl(=1)vc
...
class SpatioConvLayer(nn.Module):
    ...
    def forward(self, x, Lk):
        x_c = torch.einsum("knm,bitm->bitkn", Lk, x)  #k是图核的大小
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        x = torch.tanh(self.fc1(x)) # nlvc->nclv
        x = self.fc2(x) # nclv->nlvc
        return x
