...
class trainer():
    def __init__(self, scaler, model,student_model, **optim_args):
        ...
        # Adam optimizer
        self.optimizer      = optim.Adam(self.student_model.parameters(), lr=self.lrate, weight_decay=self.wdecay, eps=self.eps)
        # learning rate scheduler
    ...
    def train(self, input, real_val, **kwargs):
        ...
        # print(output.size())
        output_teacher  = output.transpose(1,2)

        # student
        output_student=self.student_model(input)
        # curriculum learning
        ...
        if kwargs['_max'] is not None:  # traffic flow
            predict     = self.scaler(output_student.transpose(1,2).unsqueeze(-1), kwargs["_max"][0, 0, 0, 0], kwargs["_min"][0, 0, 0, 0]).transpose(1, 2).squeeze(-1)
            teacher_val=self.scaler(output_teacher.transpose(1,2).unsqueeze(-1), kwargs["_max"][0, 0, 0, 0], kwargs["_min"][0, 0, 0, 0]).transpose(1, 2).squeeze(-1)

            real_val    = self.scaler(real_val.transpose(1,2).unsqueeze(-1), kwargs["_max"][0, 0, 0, 0], kwargs["_min"][0, 0, 0, 0]).transpose(1, 2).squeeze(-1)
            mae_loss    = 0*self.loss(predict[:, :self.cl_len, :], teacher_val[:, :self.cl_len, :])+self.loss(predict[:, :self.cl_len, :], real_val[:, :self.cl_len, :])
        else:
            ## inverse transform for both predict and real value.
            predict     = self.scaler.inverse_transform(output_student)
            teacher_val = self.scaler.inverse_transform(output_teacher)
            real_val    = self.scaler.inverse_transform(real_val[:,:,:,0])
            ## loss
            mae_loss    = 0*self.loss(predict[:, :self.cl_len, :], teacher_val[:, :self.cl_len, :], 0)+self.loss(predict[:, :self.cl_len, :], real_val[:, :self.cl_len, :], 0)
        loss = mae_loss
        loss.backward()
        ...
        # metrics
        mape = masked_mape(predict,teacher_val,0.0)
        rmse = masked_rmse(predict,teacher_val,0.0)
    ...
    def eval(self, device, dataloader, model_name, **kwargs):
        ...
        for itera, (x, y) in enumerate(dataloader['val_loader'].get_iterator()):
            ...
            # student
            output_student = self.student_model(testx)
            # scale data
            if kwargs['_max'] is not None:  # traffic flow
                ## inverse transform for both predict and real value.
                predict = self.scaler(output_student.transpose(1,2).unsqueeze(-1), kwargs["_max"][0, 0, 0, 0], kwargs["_min"][0, 0, 0, 0])
                teacher_val = self.scaler(output_teacher.transpose(1, 2).unsqueeze(-1), kwargs["_max"][0, 0, 0, 0],
            ...
            # metrics
            loss = 0*self.loss(predict, teacher_val, 0.0).item()+self.loss(predict, real_val, 0.0).item()
            mape = masked_mape(predict,teacher_val,0.0).item()
            rmse = masked_rmse(predict,teacher_val,0.0).item()
    ...
    @staticmethod
    def test_student(model,save_path_resume, device, dataloader, scaler, model_name, save=True, **kwargs):
        # test
        model.eval()
        outputs = []
        realy   = torch.Tensor(dataloader['y_test']).to(device)
        ...
        for itera, (x, y) in enumerate(dataloader['test_loader'].get_iterator()):
            ...
            with torch.no_grad():
                ...
                # output_teacher = output.transpose(1, 2)
                # student
                preds = model(testx).transpose(2, 1)
